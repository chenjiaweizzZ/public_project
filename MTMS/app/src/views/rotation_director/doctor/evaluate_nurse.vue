<template>
    <div class="g-main">
        <ul class="list" v-if="!nocontent">
            <li v-for="(item,index) in evaluatedlist" @click="detail(item.id)">
                <span>{{item.uname}}</span>
                <span>{{item.showpullscore | score}}</span>
                <span class="choice">{{item.evamonth}}</span>
            </li>
        </ul>
    ...
    export default {
        ...
        methods: {
            ...
            init: function () {
                ...
                self.post('/turnevaluate/getuserevaluatelist', self.data)
                    .done(function (data) {
                        if(data){
                            if(data.errcode){
                                if(data.errcode == 0){
