    ...
    <div class="g-main">
      ...
      <div class="main assignMain" :class="{'stopscroll': isscroll}" v-show="assingning">
        <div v-for="(item,key) in assignteachlist" class="list">
          ...
          <!--<div class="footerBtn">
                        ...
                        <a v-if="showck" :class="{noclick:issubmit}" @click='finishTrain(item.tpdid,item.num,item.id)' v-else>出科确认</a>
          </div>-->
        </div>
      </div>
      <!-- 轮转中 -->
      ...
      <div class="main allTurnMain" :class="{'stopscroll': isscroll}" v-show="allTurn">
        <div v-for="(item,key) in turnList" class="list">
          ...
          <!--<div class="footerBtn">
                        <a @click='allotTeacher(item.tpdid,item.turningdeptid)' style="margin-right: 0.2rem">修改带教老师</a>
                        <a v-if="showck" :class="{noclick:issubmit}" @click='finishTrain(item.tpdid,item.num,item.id,item.endtime)'>出科确认</a>
          </div>-->
        </div>
      </div>
      <!-- </transition> -->
