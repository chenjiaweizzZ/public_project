    ...
    <div class="g-main">
      <div v-for="item in list" class="contentDiv">
        ...
        <ul
          ...
          @click="detail(item.turnstatus,item.plandetailid,item.planendtime,item.deptid,item.teachingid)"
          class="list"
        >
          <li class="listTitle">
        ...
        <ul v-else class="list">
          ...
          <li v-if="item.turnstatus == 0 && item.turnchangeflag == 1">
            <div class="enter">
              <a @click="toelectiveoffice(item)">选择科室</a>
            </div>
          </li>
        </ul>
      </div>
...
export default {
  ...
  methods: {
    ...methods,
    ...mapActions(["setTurninfo", "setTurnstatus"]),
    //选修科室
    toelectiveoffice: function(item) {
      let self = this;
      this.$router.push({
        name: "fdRotation_doctor_elective",
        query: {
          id: this.turninfo.stuid,
          turnplanid: item.plandetailid,
    ...
    // 点击每一个具体的详情内容
    detail: function(index, param, params, deptid, teachingid) {
      //({0:'未轮转'}{1:'轮转中'}{3:'已轮转'});
      let self = this;
      this.setTurnstatus(index); //加入turnstates到vuex;
