<template>
  <div class="g-wrapper">
    <c-header v-show="showEvaluate">
      ...
      {{title}}
      <!--住院生为带教老师评分-->
      <div slot="headerRight" style="font-size: 0.3rem" @click="evaluateTeacher">评分</div>
      <!-- v-show="showsubmit" -->
    </c-header>
    <div class="g-main">
      <!--日常考核查看-->
      ...
      if (val !== oldVal) {
        if (self.$route.query.booking == 0) {
          ...
          if (self.$route.query.param == 1) {
            ...
            self.evaluateMonth(5, $.cookie("uid"), self.$route.query.deptid);
          }
        }
      }
    }
  ...
  methods: {
    ...
    evaluateTeacher: function() {
      ...
      if (self.$route.query.param == 1) {
        ...
        opitions.type = 1;
        if (Number(self.$route.query.teachingid) == 0) {
          Toast("没有被评价带教老师");
          return;
        }
      } else if (self.$route.query.param == 5) {
        ...
        opitions.type = 5;
        if (Number(self.$route.query.deptid) == 0) {
          Toast("没有被评价科室");
          return;
        }
      }
