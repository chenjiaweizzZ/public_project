<template>
  <div class="g-wrapper">
    ...
    <div class="g-main" v-if="!noContent">
      <div v-for="(item,key) in aturndoctors" class="list">
        <ul @click="plan(item)">
          <li class="head">
            <div class="headDiv">
        ...
        <div class="footerBtn">
          ...
          <a :class="{noclick:issubmit}" @click="confirmCourse(item)" v-if="!issubmit">入科确认</a>
          <a :class="{noclick:issubmit}" v-else>入科确认</a>-->
        </div>
      </div>
    </div>
    ...
    <div class="for-reason" v-if="showreason">
      <div class="reason-content">
        <header>请选择未轮转原因</header>
        <section>
          <label v-for="(v,index) in reasonlist" @click.prevent="getchecked(v.key)">
            <input type="checkbox" class="radio" :checked="v.ischecked == true" />
            {{v.name}}
          </label>
    ...
    <!--选择要看的时间-->
    <c-timepicker :config="gettime" @confirm="selecttime" :year="defaultyears" :month="months"></c-timepicker>
...
export default {
  ...
  mounted() {
    console.log(111);
    let self = this;
    if (self.$route.query.deptid && self.$route.query.where == 2) {
    ...
    //   self.init();
    //   self.initreason();
    //   console.log("111", self.aturndoctors);
    // });
  },
  methods: {
    ...methods,
    ...
    init(date) {
      ...
      self
        .post("/turn/querytoturndoctors", {
          ...
          turnstatus: 3, //未轮转：4 ； 待入科：0 ；
          month: date,
          includeChildDeptFlag: "0"
        })
        .done(function(data) {
          self.aturndoctors = data.turndoctors;
          if (self.aturndoctors.length == 0) {
            self.noContent = true;
          } else {
