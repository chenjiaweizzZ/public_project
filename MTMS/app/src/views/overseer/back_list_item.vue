<template>
  <div class="supervise-grade">
    <van-nav-bar title="督导评分详情" left-arrow @click-left="back"></van-nav-bar>
    <div class="g-main">
      <div class="header">
        <div>
      ...
      <div class="evaluateDetail" :class="{ evaDetail : type == 1 }" v-if="gradeList.length > 0">
        <div v-for="(item,index) in gradeList" class="grade-box" :key="index">
          ...
          <div v-for="(iten,indey) in item.questionList" class="content-box" :key="indey">
            ...
            <div v-if="iten.type == 5 && type == 0" style="margin-top: 10px">
              <span
                v-for="(iteb,indez) in iten.items"
            ...
            <div v-if="iten.type == 5 && type == 1" style="margin-top: 10px">
              <span
                v-for="(iteb,indez) in iten.items"
                class="selBox"
                :class="{ active: iteb.id == iten.save }"
      ...
      <div class="nodata" v-else>
        ...
        <div v class="nodataText">该专家还未评分</div>
      </div>
      <div style="height:100px;background: #fff;"></div>
    </div>
    <div class="btnGroup">
      <van-button plain type="info" @click="fkk()" v-if="!fk">反馈</van-button>
      <van-button type="info" @click="fkk()" v-else>反馈详情</van-button>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    getEvaList() {
      ...
      if (self.$route.query.type == 2) {
        type = 1;
      } else if (self.$route.query.type == 4) {
        type = 3;
      } else if (self.$route.query.type == 6) {
        type = 5;
      }
      let params = {
