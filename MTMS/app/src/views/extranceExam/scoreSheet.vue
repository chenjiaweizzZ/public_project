        ...
        <div class="g-main">
            ...
            <div class="score-content"  v-if="!loading">
                <div class="scoreItem reset">
                    <score-item v-for="(item,index) of itemlist" :key="item" :datalist="item" :index="index" @get_result_all="get_result_all" :step="step" ></score-item>
                </div>
                <div class="comment">
                    <p>评语：</p>
                ...
                <div class="signature-btn"  @click="submitItem">
                    <!-- <button id="clearCanvas" ref="clearCanvas"  style="margin-left:0.2rem" v-if="$route.query.detail">重新签名</button> -->
                </div>
            </div>
        ...
        methods:{
            ...
            stdInfo(){
                this.post('/turnExamInfo/queryexamineeinfo',{
                    ...
                    if(res&&res.errcode==0){
                        ...
                        // this.$set(this.datalist,'turnofficename',this.$route.query.deptname);
            ...
            submitItem(){//提交评分项
                ...
                if(!this.isHasComment){
                    if(Number(this.totalscore)>Number(this.defaultscore)){
                        this.$toast("分数不能大于总分");
                        return ;
                    }
                }
            ...
            onInput(value) {
                if(this.numberdistinction=='score'){
                    this.totalscore = (String(this.totalscore) + String(value)).slice(0, 6);
                }
            ...
            postSubmit(signature){
                ...
                if(this.isHasComment){
                    ...
                    if(typeof(this.$route.query.seiid) === "string") {
                        console.log(111,this.$route.query.seiid)
                        seiIds = this.stringToArray(this.$route.query.seiid)
                    }else {
                        console.log(222,this.$route.query.seiid)
            ...
            submit(signature){
                ...
                this.issubmit=true;
                // if(signature.match('http')!=null){
                    this.postSubmit(signature)
                // }else{
                //     var file=this.dataURLtoFile(signature);
                //     var furl = new FormData();
                //     furl.append('file',file);
                //     console.log(furl)
                //     let signatureUrl='';
                //     $.ajax({
                //         contentType: false,
                ...
                //         success:(data)=>{
                //             if(data.code==200){
                //                 signatureUrl=data.data.url;
                //                 this.postSubmit(signatureUrl)
