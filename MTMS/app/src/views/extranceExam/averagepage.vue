<template>
    <div class="g-wrapper">
        ...
        <div class="top">
            ...
            <div class="score">
                <div class="left">
                    <p>{{$route.query.examTypeName}}平均分： </p>
                </div>
                <div class="right">
                    <p>{{averagescore.toFixed(1)}}分</p>
                </div>
            </div>
            <div class="scoreitemslist">
                <div v-for="(item,index) of scoreitemlist" class="scoreitems"  :class="{active:itemIndex==index}" :key="index" @click.stop="getItemscore(index)">
                    ...
                    <p v-if="item.score||item.score==0">{{item.score}}分</p>
                    <p v-else>&nbsp</p>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        mounted(){
            ...
            this.scoredata=JSON.parse(this.$route.query.score);
            this.scoreitemlist=this.scoredata.itemscorelist;
            this.averagescore=this.scoredata.score;
            this.init(0);
            this.getStdInfo();
        ...
        methods:{
            ...
            getStdInfo(){
                this.post('/turnExamInfo/queryexamineeinfo',{
                    ...
                    sessionid:$.cookie('sid'),
                    loginid:$.cookie('uid'),
                    examineeid:this.scoredata.stuid
                }).done(res => {
                    this.loading=false;
                    if (res && res.errcode == 0) {
                        this.stdinfo=res;
