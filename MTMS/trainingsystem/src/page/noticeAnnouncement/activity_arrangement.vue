<template>
    <div class="wapper">
        <div v-if="show == 1">
            ...
            <el-pagination class="pageclass" style="float:right;margin:10px 10px" @size-change="handleSizeChange" @current-change="handleCurrentChange"
        ...
        <div v-if="show == 2">
            ...
            <div class="filestyle">
                ...
                <div v-for="(item, index) in detailfilelist" :key="index">
                    <a target="_blank" :href="'/file/download2?file=' + fileurltitle + item.url + '&name=' + item.name" :title="item.name">{{
                        item.name }}</a>
                </div>
            </div>
        </div>
        ...
        methods: {
            inittable: function (pageno, pagesize) {            //公告列表
                ...
                post('/consult/list', {
                    ...
                    pageno: pageno,
                    pagesize: pagesize,
                    type: 8,//2通知公告；3政策文件 7下载专区，8教学活动安排
                    status: 1,
                    title: ''
                }).then(function (data) {
