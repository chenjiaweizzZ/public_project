...
var xiefn={
    ...
    timelistsortnew:function(arr,reversesort){//2017-10-05 08:00:00格式时间数组排序，reversesort表示从大到小排
        var sortedarr=[]
        if(!reversesort){//默认从小到大排
            sortedarr=arr.sort(function(a,b){
            ...
            sortedarr=arr.sort(function(a,b){
                return Date.parse(new Date(b))-Date.parse(new Date(a))
            })
        }
        return sortedarr
    },
    seltimelistsort:function(arr,reversesort){//2017-10-05 08:00:00格式时间数组排序，reversesort表示从大到小排
        var sortedarr=[]
        if(!reversesort){//默认从小到大排
            sortedarr=arr.sort(function(a,b){
                return Date.parse(new Date(a.day + ' ' + a.starttime))-Date.parse(new Date(b.day + ' ' + b.starttime))
            })
        }else{//从小到大排
            sortedarr=arr.sort(function(a,b){
                return Date.parse(new Date(b.day + ' ' + b.starttime))-Date.parse(new Date(a.day + ' ' + a.starttime))
            })
        }
        return sortedarr
    },
