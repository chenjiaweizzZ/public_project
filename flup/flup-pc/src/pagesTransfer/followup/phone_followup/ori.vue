        ...
        <template v-else>
                    ...
                    <div class="remarkTime">
                      <div class="remarkTime-fline">
                        ...
                        <span class="remarkTime-btn">
                          <el-button type="text" @click="editRemark(item, index)" v-if="item.remarkType == 1">编辑</el-button>
          ...
          <el-row v-if="!hasComplete" class="margin-top20" type="flex" justify="center">
            ...
            <div v-else>
              ...
              <el-button v-if="!hasOverdue" class="width200px" type="primary" @click.once.stop.prevent="onSubmit">提
                交</el-button>
            </div>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    changePatient(v) {
      ...
      this.hasComplete = v.flupState === "已完成" || v.flupState === "已作废";
    ...
    beforeAvatarUpload(file) {
      let audioFileType = ['audio/mpeg', 'audio/x-wav'];
    ...
    handleAvatarSuccess(res) {
      let { retData } = res
      if (retData) {
        console.log(this.$baseURL.ICDMApiUrl + "/file/audio/" + retData.id)
        this.$apis
          .insertPatientRemark({
            userId: this.userId,
