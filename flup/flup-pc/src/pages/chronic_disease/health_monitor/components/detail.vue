  ...
  <div class="chart-detail">
    ...
    <el-row class="margin-top20" type="flex">
      <div class="flex1 border1 padding20">
        ...
        <el-row class="pre-wrap">
          {{ `上传总次数：${dataList.length} 上次${title}：${lastData}` }}
        </el-row>
      </div>
...
export default {
  ...
  computed: {
    ...
    lastData() {
      ...
      return getLastData(
        this.dataList,
        enumData[this.type].valueKey,
        enumData[this.type].unitKey,
        enumData[this.type].unit
      )
    },
  },
  data() {
  ...
  methods: {
    ...
    formatState(row, col, val) {
      return row.state === 0 ? '正常' : row.warningContent
    },
    formatBetween(row) {
