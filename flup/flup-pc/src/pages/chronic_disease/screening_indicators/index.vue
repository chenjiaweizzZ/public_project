<template>
  <div class="screening_indicators">
    <div class="header">
      <div class="search">
        <span>搜索</span>
    ...
    <div class="main">
      ...
      <div class="positive-table" v-if="total > 0">
        <el-table :data="tableData" border style="width: 100%">
          ...
          <el-table-column prop="ruleName" label="筛选指标名称" show-overflow-tooltip></el-table-column>
          ...
          <el-table-column label="生效范围" show-overflow-tooltip>
            <template slot-scope="scope">
              ...
              ? "单次就诊"
              :scope.row.effectiveScope == '1' ?"单次体检":''
              }}
            </template>
          </el-table-column>
          <el-table-column label="操作" width="200">
            <template slot-scope="scope">
              ...
              <el-button type="text" @click="filter(scope.row)">筛选</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
    ...
    <el-dialog :title="title" :visible.sync="dialogVisible" :close-on-click-modal="false" width="744px" :before-close="handleClose" class="indicators-dialog">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm">
        ...
        <el-form-item label="生效范围" prop="effectiveScope" label-width="100px">
          <el-radio-group v-model="ruleForm.effectiveScope">
            <el-radio label="0">单次就诊</el-radio>
            <el-radio label="1">单次体检</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="健康管理组" prop="effectiveScope" label-width="100px">
          <el-select v-model="ruleForm.flupGroup" multiple filterable placeholder="请选择" @visible-change="show($event,)" @change="changeFlupGroup">
            <el-option v-for="item in flupGroupList" :key="item.serialNo" :label="item.groupName" :value="item.serialNo"></el-option>
          </el-select>
        </el-form-item>
        <div class="key">筛查条件</div>
...
export default {
  ...
  methods: {
    ...
    getPositiveIndicatorRuleDetail(serialNo) {
      this.$apis
        ...
        .then((res) => {
          ...
          this.ruleForm = res;
          this.ruleForm.flupGroup = res.groupIds ? res.groupIds.split(',') : [];
    ...
    Add() {
      this.title = '新增筛查指标';
      this.dialogVisible = true;
      if (this.flupGroupList.length == 0) {
     ...
     */
    Delete(row) {
      this.$confirm('是否删除该条筛选标准，删除后，已经筛选的列表不会消失', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        closeOnClickModal: false,
        customClass: 'screening_indicators',
      })
        .then(() => {
          this.$apis
        ...
        if (valid) {
          this.$apis
            .savePositiveIndicatorRule({
              ...
              effectiveScope: this.ruleForm.effectiveScope,
              ...
              ruleType: 2,
              groupIds: this.ruleForm.flupGroup.join(','),
            })
            .then((res) => {
              this.$message({
                type: 'success',
