<template>
  <div class="discharge-info">
      ...
      <el-col class="flex">
        ...
        <div class="value">{{
            basicInfo.dateOfBirth
              ? this.$utils.formatDate(basicInfo.dateOfBirth)
              : ""
          }}</div>
      </el-col>
    </el-row>
    <el-row class="margin-top16" type="flex">
      ...
      <div class="value">{{ basicInfo.clinicalDiagnosis }}</div>
    </el-row>
    <el-row class="margin-top16" type="flex">
      <div class="label">检查</div>
      <div class="value">{{ basicInfo.examNames }}</div>
    </el-row>
    <el-row class="margin-top16" type="flex">
      <div class="label">检验</div>
      <div class="value">{{ basicInfo.labNames }}</div>
    </el-row>
    <el-row class="margin-top16" type="flex">
      <div class="label">处方</div>
      <div class="value">{{ basicInfo.prescribeShow }}</div>
    </el-row>
    <el-row class="margin-top16" type="flex">
      <div class="label">医生</div>
      <div class="value">{{ basicInfo.medicalDoctor }}</div>
    </el-row>
    <el-row class="margin-top16" type="flex">
      <div class="label">科室</div>
      <div class="value">{{ basicInfo.medicalDept }}</div>
    </el-row>
    <el-row class="margin-top16" type="flex">
      <div class="label">时间</div>
      <div class="value">{{
          basicInfo.medicalDate
            ? this.$utils.formatDate(basicInfo.medicalDate)
            : ""
        }}</div>
    </el-row>
    <!-- <el-row class="margin-top16" type="flex">
      <el-col class="flex">
        <div class="label">门诊号</div>
      ...
      async handler() {
        if (this.serialNo) {
          await this.$apis
            ...
            .then(res => {
              this.basicInfo = res;
              this.basicInfo.prescribeShow = "";
              if (this.basicInfo.prescribe && this.basicInfo.prescribe.length) {
                for (
                  let index = 0;
                  index < this.basicInfo.prescribe.length;
                  index++
                ) {
                  this.basicInfo.prescribeShow +=
                    this.basicInfo.prescribe[index].oderName.trim() +
                    "," +
                    this.basicInfo.prescribe[index].jiliang.trim() +
                    "," +
                    this.basicInfo.prescribe[index].interval.trim() +
                    "；" +"\n";
                }
              }
