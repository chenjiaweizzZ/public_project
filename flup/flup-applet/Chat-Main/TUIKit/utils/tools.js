...
export async function formatMessageList(messageList) {
  ...
  console.log('去重的messageList: ', messageList)
  // 对数据按时间排序
  ...
  for (let i = 0; i < messageList.length; i++) {
    ...
    if (!wx.getStorageSync('lose')) {
      wx.setStorageSync('lose', [])
    }
    let lose = wx.getStorageSync('lose')
    ...
    if (message.type == 'TIMImageElem') {
      message.bigImage = await getApp().getPrivateCosSignImage(message.bigImage)
      message.oriImage = await getApp().getPrivateCosSignImage(message.oriImage)
      message.smallImage = await getApp().getPrivateCosSignImage(message.smallImage)
    }
  }
...
export function formatMessage(message) {
  ...
  if (message.type === "TIMImageElem" && message.payload && message.payload.imageInfoArray) { //图片
...
audioContext.onPause(onAudioPause)
audioContext.onPlay(onAudioPlay)
audioContext.onEnded(onAudioEnded)
var audioCallback = null
var audioPlayStatus = '' // playing  stop  pause  ended
export function playVoiceMessage(url, callback) {
...
export function stopCurrentVoiceMessage() {
  audioContext.stop()
}

function onAudioStop() {
  audioPlayStatus = 'stop'
  if (audioCallback) {
    audioCallback('stop')
  }
}
