...
const cdnConfig = [
  ...
  //   cndHttp: 'https://cdn.bootcdn.net/ajax/libs' // 当不使用默认的cnd镜像源时，可以配置这个
  // },
  ...
  //   cndHttp: 'https://cdn.bootcdn.net/ajax/libs'
  // },
  // {
  //   name: 'vconsole',
  //   scope: 'VConsole',
  ...
  //   js: 'vant.min.js',
  //   css: 'index.css',
  //   cndHttp: 'https://cdn.jsdelivr.net/npm'
  // }
]
  ...
  config.forEach(item => {
    ...
    if (item.name in dependencieModules) {
      ...
      if (item.name == 'vant') {
        item.css =
          item.css &&
          [currCdnHttp, item.name + '@' + version, 'lib', item.css].join('/')
      } else {
        // 拼接完整 css 的链接
        item.css =
          item.css &&
      ...
      if (item.name == 'vconsole') {
        item.js =
          item.js && [currCdnHttp, 'vConsole', version, item.js].join('/')
      } else if (item.name == 'vant') {
        item.js =
          item.js && [currCdnHttp, item.name + '@' + version, 'lib', item.js].join('/')
      } else {
        // 拼接完整 js 的链接
        item.js =
