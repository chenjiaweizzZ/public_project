...
import { isUndefined } from '@/utils/trtcUtils.js';
...
export default {
  ...
  methods: {
    ...
    async join() {
      ...
      try {
        await this.client.join({ roomId: Number(this.roomId)});
        // await this.client.join({ roomId: 123});
