<template>
  <div class="login">
    <div class="login-info">
      ...
      <img
        class="login-info__img"
        v-else
        src="@/assets/images/core/headSculpture.png"
        alt=""
      />
      <span class="login-info__text">{{ nickName }}</span>
      <span class="login-info__introduce mt">亲爱的微信平台用户</span>
      <span class="login-info__introduce"
        >为了给您更好的服务，请输入您的手机号</span
      >
    </div>
    <el-form
      :model="ruleForm"
          ...
          >
            <div slot="suffix" class="code">
              <span
                v-if="!isSuccess"
                class="code-text"
                @click.stop="handleCanvas"
                >获取验证码</span
              >
              <span v-else class="code-textDisabled"
                >重新获取({{ codeTime }})</span
              >
            </div>
          </el-input>
    ...
    <div class="login-article">
      <van-checkbox
        v-model="isArticle"
        shape="square"
        checked-color="#32ae57"
        icon-size="16px"
      >
        点击勾选即表示同意<span
          class="login-article__text"
          @click.stop="isUserAgreement = true"
          >服务协议</span
        >和<span class="login-article__text" @click.stop="isProvision = true"
          >隐私条款</span
        >
      </van-checkbox>
    </div>
    ...
    <van-popup v-model="isUserAgreement" class="login-popup">
      <div class="login-userAgreement">
        <van-icon
          name="cross"
          class="login-userAgreement__icon"
          @click="isUserAgreement = false"
        />
...
服务机构：卓健数智
    ...
    <van-popup v-model="isProvision" class="login-popup">
      <div class="login-Provision">
        <van-icon
          name="cross"
          class="login-Provision__icon"
          @click="isProvision = false"
        />
...
import verificatonCode from "./components/verificatonCode.vue";
...
export default {
  ...
  mounted() {
    if(localStorage.getItem('openId') || JSON.parse(localStorage.getItem('userInfo'))) {
      console.log(111)
      document.title = "绑定手机";
