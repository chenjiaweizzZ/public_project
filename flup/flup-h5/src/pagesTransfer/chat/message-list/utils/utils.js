...
export function handleName(item) {
  const { t } = TUIKitTUICore.config.i18n.useI18n();
  let name = '';
  switch (item.type) {
    case TIM.TYPES.CONV_C2C:
...
export function handleReferenceForShow(message) {
  const data = {
...
export function handleShowLastMessage(item) {
  const { t } = TUIKitTUICore.config.i18n.useI18n();
...
export function handleTipMessageShowContext(message) {
  const { t } = TUIKitTUICore.config.i18n.useI18n();
  const options = {
    message,
    text: '',
...
export function extractCallingInfoFromMessage(message) {
  const { t } = TUIKitTUICore.config.i18n.useI18n();
  let callingMessage = {};
  let objectData = {};
  try {
...
export function handleOptions(businessID, version, other) {
  return {
    businessID,
    version,
    ...other,
...
export function isTypingMessage(item) {
...
export const isMessageTip = (message) => {
  if (
...
export const handleSkeletonSize = (width, height, maxWidth, maxHeight)=> {
