    ...
    <div class="search-section">
      ...
      <div class="search-section-item">
        ...
        <el-input v-model.number="search.patientPhone" class="w-50 m-2" size="large" placeholder="请输入" maxlength="11"
          style="width:170px" @input="validateInput" clearable/>
      </div>
      <div class="search-section-item">
    ...
    <div class="table-container">
      <el-table :data="state.tableData" border style="width: 100%" :show-overflow-tooltip="true">
        ...
        <el-table-column fixed="right" label="操作" width="220">
          <template #default="scope">
            <el-button v-if="getAction(scope.row.actionCodes,'SKIM')" link type="primary" size="small" @click="handle(scope.row.recordId,true,'住院转入查看',2)">查看</el-button>
            <el-button v-if="getAction(scope.row.actionCodes,'EXIT')" link type="primary" size="small" @click="handle(scope.row.recordId,false,'住院转入编辑')">编辑</el-button>
            <el-button v-if="getAction(scope.row.actionCodes,'REVIEW')" link type="primary" size="small" @click="handle(scope.row.recordId,false,'住院转入审核',4)">审核</el-button>
...
// })
let search = store.beinHospitalToSeatch
...
const getTransferStatusList = () => {
  getTransferStatus().then(res => {
    if (utils.isArray(res)) {
      options.procesStatusList = res.filter(i => i.statusType == '1' && i.transferType == 'ZY')   //住院流程状态
      options.businesStatusList = res.filter(i => i.statusType == '2' && i.transferType == 'ZY')   //住院业务状态
      getTableList()
    }
  })
}
const getTableList = () => {
  search.enterPageType = 4
...
const handle = (recordId = '', isSee = false, title, operateType = 1) => {
  ...
  window.parent.postMessage(
        JSON.stringify({
          method: "addTab",
          url: `${urlx}#/beInHospital?type=2&recordId=${recordId}&isSee=${isSee}&operateType=${operateType}`,
          title
        }),
    "*"
  );
