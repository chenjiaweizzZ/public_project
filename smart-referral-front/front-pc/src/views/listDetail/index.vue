...
<template>
  <div class="list-detail" :style="{ paddingBottom: !isTransferIn && !isAddTransferOut ? '0px' : '' }">
      ...
      <el-form ref="ruleFormRef1" :model="ruleForm" :rules="rules" label-width="120px">
        <template class="flex">
          <el-form-item class="flex-item" label="病人姓名" prop="patientName">
            <el-input v-model="ruleForm.patientName" :placeholder="isSee == 'true' || operateType == 4 ? '' : '病人姓名'" maxlength="20" :disabled="isDisabled" />
          </el-form-item>
          <el-form-item class="flex-item" label="证件类型" prop="idCardType">
            <el-select v-model="ruleForm.idCardType" :placeholder="isSee == 'true' || operateType == 4 ? '' : '请选择'" :disabled="isDisabled" @change="idCardTypeChange">
              <el-option v-for="item in documentList" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
          <el-form-item class="flex-item" label="证件号码" prop="idNo">
            <el-input v-model="ruleForm.idNo" :placeholder="isSee == 'true' || operateType == 4 ? '' : '证件号码'" maxlength="18" :disabled="isDisabled" />
          </el-form-item>
        </template>
        <template class="flex">
          ...
          <el-form-item class="flex-item" label="病人年龄" prop="patientAge">
            <el-input v-model.number="ruleForm.patientAge" @input="ruleForm.patientAge = String(ruleForm.patientAge).replace(/[^\d]/g, '')" :placeholder="isSee == 'true' || operateType == 4 ? '' : '病人年龄'" maxlength="20" :disabled="isDisabled" :pattern="[0 - 9]" />
          </el-form-item>
          <el-form-item class="flex-item" label="手机号码" prop="patientPhone">
            <el-input v-model.number="ruleForm.patientPhone" @input="ruleForm.patientPhone = String(ruleForm.patientPhone).replace(/[^\d]/g, '')" :placeholder="isSee == 'true' || operateType == 4 ? '' : '手机号码'" maxlength="11" :disabled="isDisabled" />
          </el-form-item>
        </template>
      </el-form>
    </detailItem>
      ...
      <el-form ref="ruleFormRef2" :model="ruleForm" :rules="rules" label-width="120px">
        <div class="flex">
          ...
          <el-form-item class="flex-item" label="医保卡卡号">
            <el-input v-model="ruleForm.clinicCardNo" :placeholder="isSee == 'true' || operateType == 4 ? ' ' : '请输入'" maxlength="20" :disabled="isDisabled" />
          </el-form-item>
          <el-form-item class="flex-item" label="申请人" prop="applyUserName">
            <el-input v-model="ruleForm.applyUserName" :placeholder="isSee == 'true' || operateType == 4 ? ' ' : '请输入'" maxlength="20" :disabled="isDisabled" />
          </el-form-item>
        </div>
        <div class="flex">
          ...
          <el-form-item class="flex-item" label="用户ID号">
            <el-input v-model="ruleForm.patientUserId" :placeholder="isSee == 'true' || operateType == 4 ? ' ' : '请输入'" maxlength="20" :disabled="isDisabled" />
          </el-form-item>
          <el-form-item class="flex-item" label="申请人电话" prop="applyPhone">
            <el-input v-model.number="ruleForm.applyPhone" @input="ruleForm.applyPhone = String(ruleForm.applyPhone).replace(/[^\d]/g, '')" :placeholder="isSee == 'true' || operateType == 4 ? ' ' : '请输入'" maxlength="11" :disabled="isDisabled" />
          </el-form-item>
        </div>
        <div class="flex">
          ...
          <!-- prop="sourceDeptId" -->
          <el-form-item class="flex-item" label="转出科室">
            <!-- <el-input
              v-model="ruleForm.sourceDeptName"
              :placeholder="
                isSee == 'true' || operateType == 4 ? ' ' : '请输入'
              "
              :disabled="isDisabled"
            >
            </el-input> -->
            <el-select v-model="ruleForm.sourceDeptId" allow-create filterable :placeholder="isSee == 'true' || operateType == 4 ? ' ' : '请选择'" :disabled="isDisabled" @change="Change1">
          ...
          <el-form-item class="flex-item" label="转入医院" prop="targetHospitalId">
            <el-select v-model="ruleForm.targetHospitalId" filterable :placeholder="isSee == 'true' || operateType == 4 ? ' ' : '请选择'" :disabled="isDisabled" @change="Change2">
        ...
        <div class="flex">
          <el-form-item class="flex-item" label="转入科室" prop="targetDeptId">
            <el-select v-model="ruleForm.targetDeptId" filterable :placeholder="isSee == 'true' || operateType == 4 ? ' ' : '请选择'" :disabled="isDisabled" @change="Change3">
              <el-option v-for="item in departmentList" :key="item.SectionId" :label="item.SectionName" :value="item.SectionId" />
            </el-select>
          </el-form-item>
          <el-form-item class="flex-item" label="时间段" prop="expectedDateSpan">
            <el-select v-model="ruleForm.expectedDateSpan" :placeholder="isSee == 'true' || operateType == 4 ? ' ' : '请选择'" @change="Change4" :disabled="isDisabled">
              <!-- <el-option label="全天" value="ALL" /> -->
              <el-option label="上午" value="AM" />
              <el-option label="下午" value="PM" />
            </el-select>
          </el-form-item>
          <el-form-item v-if="transferType == 'MZ' && ruleForm.targetHospitalId == '1118033010334208499'" class="flex-item" label="期望日期" prop="time">
            <el-date-picker style="width: 100%" v-model="ruleForm.time" type="date" :placeholder="isSee == 'true' || operateType == 4 ? '' : '选择日期'" format="YYYY-MM-DD" value-format="YYYY-MM-DD" :disabled="isDisabled" :disabled-date="disabledDate" @change="Change4">
            </el-date-picker>
          </el-form-item>
          <el-form-item v-else class="flex-item" label="期望日期">
            <el-date-picker :disabled="isDisabled" v-model="ruleForm.time" type="daterange" range-separator="至" start-placeholder="开始日期" end-placeholder="结束日期" format="YYYY-MM-DD" value-format="YYYY-MM-DD"></el-date-picker>
          </el-form-item>
        </div>
        <div class="flex">
          <el-form-item v-if="transferType == 'MZ' && ruleForm.targetHospitalId == '1118033010334208499'" class="flex-item" label="接诊医生" prop="targetDoctorId">
            <el-select v-model="ruleForm.targetDoctorId" filterable :placeholder="isSee == 'true' || operateType == 4 ? ' ' : '请选择'" :disabled="isDisabled" @change="Change5" @visible-change="checkSelect" ref="docSelect">
              <el-option v-for="item in doctorList" :key="item.doctCode + item.deptTwoName" :label="item.deptTwoName ? item.doctName + '-' + item.deptTwoName : item.doctName" :value="item.doctCode + '@#' + item.deptTwoName" />
            </el-select>
          </el-form-item>
          <el-form-item class="flex-item" label="其它备注">
            <el-input v-model="ruleForm.otherRemark" :placeholder="isSee == 'true' || operateType == 4 ? ' ' : '请输入'" maxlength="100" :disabled="isDisabled" />
          </el-form-item>
          <el-form-item v-if="!(transferType == 'MZ' && ruleForm.targetHospitalId == '1118033010334208499')" class="flex-item"></el-form-item>
    ...
    <detailCollapseItem title="主诊断" :disabled="true" :must="true">
      ...
      <el-form ref="ruleFormRef3" :model="ruleForm" :rules="rules">
        <el-form-item prop="mainDiagnosis">
          <el-input :disabled="isDisabled" v-model="ruleForm.mainDiagnosis" :placeholder="isSee == 'true' || operateType == 4 ? ' ' : '请输入'" type="textarea" rows="5" resize="none" maxlength="1000" />
        </el-form-item>
      </el-form>
    </detailCollapseItem>
    <detailCollapseItem :isShow="queryClinicReportList && queryClinicReportList.length > 0" title="门诊记录" :total="queryClinicReportList ? queryClinicReportList.length : 0" :must="true" :disabled="true">
      <!-- 表单 -->
      <el-form v-if="!queryClinicReportList || queryClinicReportList.length < 1" ref="ruleFormRef4" :model="ruleForm" :rules="rules">
        <el-form-item prop="clinicReportList">
          <el-input :disabled="isDisabled" v-model="ruleForm.clinicReportList" :placeholder="isSee == 'true' || operateType == 4 ? ' ' : '请输入'" type="textarea" rows="5" resize="none" maxlength="1000" />
        </el-form-item>
      </el-form>
      <!-- 数据展示 -->
      <!-- v-else-if="
          ...
          queryClinicReportList.length > 0
      "-->
      <expandTable v-else type="1" :isRollOut="isRollOut" :isSee="isSee" :tableData="queryClinicReportList" :selectList="deselectClinicReportList" @seeList="seeList" @select-list="selectList">
    ...
    <detailCollapseItem :isShow="queryEmrExamReportList && queryEmrExamReportList.length > 0" title="检查记录" :total="queryEmrExamReportList ? queryEmrExamReportList.length : 0">
      <!-- 表单 -->
      <el-input v-if="!queryEmrExamReportList || queryEmrExamReportList.length < 1" :disabled="isDisabled" v-model="ruleForm.emrLabReportList" :placeholder="isSee == 'true' || operateType == 4 ? ' ' : '请输入'" maxlength="1000" type="textarea" rows="5" resize="none" />
      <expandTable v-else #default="params" type="2" :isRollOut="isRollOut" :isSee="isSee" :tableData="queryEmrExamReportList" :selectList="deselectExamReportList" @select-list="selectList">
        <div class="table-detail">
          <section class="flex">
    ...
    <detailCollapseItem :isShow="queryEmrLabReportList && queryEmrLabReportList.length > 0" title="检验记录" :total="queryEmrLabReportList ? queryEmrLabReportList.length : 0">
      <!-- 表单 -->
      <el-input v-if="!queryEmrLabReportList || queryEmrLabReportList.length < 1" :disabled="isDisabled" v-model="ruleForm.examReportList" maxlength="1000" :placeholder="isSee == 'true' || operateType == 4 ? ' ' : '请输入'" type="textarea" rows="5" resize="none" />
      <!-- 数据展示 -->
      <expandTable v-else #default="params" type="3" :isRollOut="isRollOut" :isSee="isSee" :tableData="queryEmrLabReportList" :selectList="deselectEmrLabReportList" @select-list="selectList">
        <div class="table-detail">
          <el-table :data="params.data.labReport" border stripe>
            ...
            <el-table-column show-overflow-tooltip label="参考范围">
              <template slot-scope="{ row }">
                ...
                <div class="range" v-else v-html="row.referenceMin ? row.referenceMin : row.referenceMax ? row.referenceMax : row.reference ? row.reference : ''"></div>
              </template>
            </el-table-column>
          </el-table>
        </div>
    ...
    <detailCollapseItem :isShow="queryEmrAdmissionRecordList && queryEmrAdmissionRecordList.length > 0" :total="queryEmrAdmissionRecordList ? queryEmrAdmissionRecordList.length : 0" title="入院记录">
      <!-- 表单 -->
      <el-input v-if="!isSelectPatient || (isSelectPatient && (!queryEmrAdmissionRecordList || queryEmrAdmissionRecordList.length < 1))" :disabled="isDisabled" v-model="ruleForm.admissionRecordList" maxlength="1000" :placeholder="isSee == 'true' || operateType == 4 ? ' ' : '请输入'" type="textarea" rows="5" resize="none" />
      <!-- 数据展示 -->

      <expandTable v-else-if="isSelectPatient && queryEmrAdmissionRecordList && queryEmrAdmissionRecordList.length > 0" #default="params" type="4" :isRollOut="isRollOut" :isSee="isSee" :tableData="queryEmrAdmissionRecordList" :selectList="deselectAdmissionRecordList" @select-list="selectList">
        <div class="table-detail">
          <div class="flex">
            <section class="flex-item flex">
              <span class="key">姓名</span>
    ...
    <detailCollapseItem :isShow="queryEmrDischargeRecordList && queryEmrDischargeRecordList.length > 0" title="出院记录" :total="queryEmrDischargeRecordList ? queryEmrDischargeRecordList.length : 0">
      <!-- 表单 -->
      <el-input v-if="!isSelectPatient || (isSelectPatient && (!queryEmrDischargeRecordList || queryEmrDischargeRecordList.length < 1))" :disabled="isDisabled" v-model="ruleForm.dischargeRecordList" maxlength="1000" :placeholder="isSee == 'true' || operateType == 4 ? ' ' : '请输入'" type="textarea" rows="5" resize="none" />
      <expandTable v-else-if="isSelectPatient && queryEmrDischargeRecordList && queryEmrDischargeRecordList.length > 0" #default="params" type="5" :isRollOut="isRollOut" :isSee="isSee" :tableData="queryEmrDischargeRecordList" :selectList="deselectDischargeRecordList" @select-list="selectList">
        <div class="table-detail">
          <div class="flex">
            <section class="flex-item flex">
    ...
    <detailCollapseItem title="其它">
      ...
      <el-form v-else :model="ruleForm" label-width="40px">
        <el-form-item style="position: reactive">
          <template #label>
            <span @mouseenter="onMouseenter" @mouseleave="onMouseleave">图片</span>
          </template>
          <el-upload ref="uploadRefName" multiple :file-list="ruleForm.fileList" action list-type="picture-card" :before-upload="beforeUpload" :http-request="httpRequest" :on-preview="handlePictureCardPreview" :before-remove="beforeRemove" :disabled="isDisabled">
            <el-icon>
              <Plus />
            </el-icon>
          </el-upload>
...
let backups = ref([]);
let docSelectValue = ref();
const checkSelect = () => {
  if (docSelectValue.value)
    docSelect.value.selectedLabel = docSelectValue.value;
};
...
const rules = reactive({
  patientName: [
    { required: true, message: "请输入病人姓名", trigger: "change" },
  ],
  idNo: [
    {
      required: true,
  ...
  clinicCardType: [
    { required: true, message: "请选择医保卡类型", trigger: "change" },
  ],
  idCardType: [
  ...
  sourceHospitalName: [
    { required: true, message: "请输入转出医院", trigger: "change" },
  ],
  sourceDeptId: [
    { required: true, message: "请选择转出科室", trigger: "change" },
  ],
  targetHospitalId: [
    { required: true, message: "请选择转入医院", trigger: "change" },
  ],
  targetDeptId: [
    { required: true, message: "请选择转入科室", trigger: "change" },
  ],
...
const init = () => {
  ...
  if (operateType == 3 || operateType == 1 || operateType == 5) {
    const width = 160;
...
if (
  type == 1 &&
  isSee == "false" &&
  (!ruleForm.currentProcessStatus || ruleForm.currentProcessStatus == "DTJ")
) {
  ...
  outDepartmentList.value = await getCurrentUserDept(); // 迭代2
}
onMounted(() => {
...
const fillPatient = (patientId) => {
  ...
  // let params = ["20231123020008220uv7"]
  // _http.post(_apiUrl.getPatientInfoListByUserList,{params}).then(res => {
  //   let patientInfo = {}
  //   if(res.retCode == 0 && res.retData.length) {
  //     patientInfo = { ...res.retData[0] }
  ...
  _http
    ...
    .then(async (res) => {
      ...
      ruleForm.targetDoctorId =
        obj.referralRecordVo.targetDoctorId +
        "@#" +
        obj.referralRecordVo.targetDoctorDeptName;
      ruleForm.fileList =
      ...
      ruleForm.time =
        transferType == "MZ" &&
        ruleForm.targetHospitalId == "1118033010334208499"
          ? ruleForm.expectedDateFrom
          : ruleForm.expectedDateFrom && ruleForm.expectedDateTo
          ? [ruleForm.expectedDateFrom, ruleForm.expectedDateTo]
          : [];
      ...
      if (
        obj.referralRecordVo.patientId &&
        (operateType == 3 || operateType == 5)
      ) {
        // 获取门诊列表......等数据
        let params = {
        ...
        selectAdmissionRecordList.value =
          obj.recordJsonListVo.admissionRecordList;
        ...
        selectDischargeRecordList.value =
          obj.recordJsonListVo.dischargeRecordList;
        deselectClinicReportList.value = obj.recordJsonListVo.clinicReportList;
        deselectAdmissionRecordList.value =
          obj.recordJsonListVo.admissionRecordList;
        ...
        deselectDischargeRecordList.value =
          obj.recordJsonListVo.dischargeRecordList;
      }
      if (
        (obj.referralRecordVo.patientId && isSee == "true") ||
        operateType == 4 ||
        operateType == 2
      ) {
        queryClinicReportList.value = obj.recordJsonListVo
          ? obj.recordJsonListVo.clinicReportList
          : [];
        queryEmrAdmissionRecordList.value = obj.recordJsonListVo
          ? obj.recordJsonListVo.admissionRecordList
          : [];
        queryEmrExamReportList.value = obj.recordJsonListVo
          ? obj.recordJsonListVo.examReportList
          : [];
        queryEmrLabReportList.value = obj.recordJsonListVo
          ? obj.recordJsonListVo.emrLabReportList
          : [];
        queryEmrDischargeRecordList.value = obj.recordJsonListVo
          ? obj.recordJsonListVo.dischargeRecordList
          : [];
      }
      if (type == 2 || (type == 1 && isSee == "true")) {
        ...
        doctorList.value = [
          {
            doctCode: ruleForm.targetDoctorId,
            doctName: ruleForm.targetDoctorName,
          },
        ];
      }
      // 获取转出医院科室
      if (type == 1 && isSee == "false" && ruleForm.targetHospitalId) {
        if (
          ...
          departmentList.value = await getSelectAllDeptBySourceId({
            hospitalId: ruleForm.targetHospitalId,
            sectionClass: "801",
          });
        } else {
      ...
      if (
        ...
        ruleForm.expectedDateSpan &&
        !_utils.isEmpty(ruleForm.time)
      ) {
        // 获取医生列表
        getDoctorList(
          obj.referralRecordVo.targetDoctorDeptName
            ? ""
            : obj.referralRecordVo.targetDoctorName
        );
      }
    });
}
...
const setInsertReferralApply = (type) => {
  let params = {
    referralRecord: {
      ...
      expectedDateSpan: ruleForm.expectedDateSpan, // 时间段
      patientUserId: ruleForm.patientUserId, // 用户 ID 号
      visitNo: ruleForm.visitNo, // 门诊卡号
      mainDiagnosis: ruleForm.mainDiagnosis, // 主诊断,
      currentProcessStatus: ruleForm.currentProcessStatus
        ? ruleForm.currentProcessStatus
        : "", // 当前流程状态
      expectedDateFrom: _utils.isArray(ruleForm.time)
        ? ruleForm.time[0]
          ? ruleForm.time[0]
          : ""
        : ruleForm.time,
      expectedDateTo: _utils.isArray(ruleForm.time)
        ? ruleForm.time[1]
          ? ruleForm.time[1]
          : ""
        : ruleForm.time,
      ...
      transferType: transferType, // 门诊、住院
      recordId: recordId && operateType !== "5" ? recordId : "", // 转诊记录id
      targetDoctorId: ruleForm.targetDoctorId
        ? ruleForm.targetDoctorId.split("@#")[0]
        : "", // 医生ID
      targetDoctorName: ruleForm.targetDoctorName, // 医生姓名
      targetDoctorDeptId: ruleForm.targetDoctorDeptId,
      targetDoctorDeptName: ruleForm.targetDoctorDeptName,
    },
    recordTextListVo: {
  ...
  _http
    ...
    .then((res) => {
      if (res.retCode == 0) {
        ...
        let url =
          transferType == "MZ"
            ? `${urlx}#/outpatientServiceLeave?isBack=true`
            : `${urlx}#/beinHospitalLeave?isBack=true`;
        let title = transferType == "MZ" ? "门诊转出列表" : "住院转出列表";
        window.parent.postMessage(
          JSON.stringify({
            method: "addTab",
            url,
...
const submitForm = () => {
  ...
  Promise.all([p1, p2, p3, p4]).then(() => {
    if (
      queryClinicReportList.value &&
      queryClinicReportList.value.length > 0 &&
      selectClinicReportList.value.length == 0
    ) {
      ElMessage({
        type: "warning",
 ...
 * 转入拒绝同意操作相关
 */
let { dialogVisible, Refuse, handleClose, refuseConfirm, InAgree } =
  transferTn(refuseFormRef);
...
const Change2 = async (v) => {
  ...
  if (v == "1118033010334208499" && transferType == "MZ") {
    ruleForm.time = "";
  } else {
    ruleForm.time = [];
  }
  ...
  ruleForm.targetDoctorDeptId = "";
  ruleForm.targetDoctorDeptName = "";
...
const Change5 = (value) => {
  if (!ruleForm.targetDoctorId) {
    ruleForm.targetDoctorName = "";
    ruleForm.targetDoctorDeptId = "";
    ruleForm.targetDoctorDeptName = "";
    return;
  }
  console.log(doctorList.value,value)
  let arr = doctorList.value.filter((item) => {
    return item.doctCode + "@#" + item.deptTwoName == value;
  });
  console.log(arr)
  ruleForm.targetDoctorName = arr[0].doctName;
  ruleForm.targetDoctorDeptId = arr[0].deptTwoCode;
  ruleForm.targetDoctorDeptName = arr[0].deptTwoName;
  if (docSelectValue.value) docSelectValue.value = "";
};
// 获取医生列表
const getDoctorList = (selectedLabel = "") => {
  doctorList.value = [];
  _http
    .get(_apiUrl.doctorList, {
      params: {
        startDate: ruleForm.time,
        endDate: ruleForm.time,
        deptTopCode: ruleForm.targetDeptId,
        timeDescCode: ruleForm.expectedDateSpan,
      },
    })
    .then((res) => {
      if (res.retCode == 0) {
        let arr = res.retData;
        arr.forEach((i) => {
          if (i?.doctList?.DoctClinicInfo) {
            i?.doctList?.DoctClinicInfo.forEach((j) => {
              j.deptTwoName = i.deptTwoName;
              j.deptTwoCode = i.deptTwoCode;
            });
          }
        });
        let newArr = [];
        arr.forEach((item) => {
          newArr = deWeight(newArr.concat(item.doctList.DoctClinicInfo)).filter(
            (item) => {
              return item.doctName && item.doctCode;
            }
          );
        });
        doctorList.value = newArr;
        if (selectedLabel) {
          nextTick(() => {
            docSelect.value.selectedLabel = selectedLabel;
            docSelectValue.value = selectedLabel;
          });
        }
      }
    });
...
const deWeight = (arr) => {
  for (let i = 0; i < arr.length - 1; i++) {
    for (let j = i + 1; j < arr.length; j++) {
      if (
        arr[i].doctCode == arr[j].doctCode &&
        arr[i].deptTwoCode == arr[j].deptTwoCode
      ) {
        arr.splice(j, 1);
...
const uploadImg = () => {
  timer.value = setInterval(() => {
    _http
      .post(_apiUrl.getPic, { showLoading: false }, { formId: formIds.value })
      .then((res) => {
