  ...
  <div class="bein-hospital-leave">
    <div class="search-section">
      ...
      <div class="search-section-item">
        ...
        >
          <el-option
            v-for="item in options.myHosDeptList"
            :key="item.SectionId"
      ...
      <div class="search-section-item">
        ...
        <el-select
          v-model="search.targetHospitalId"
          @change="selectHosChange"
          placeholder="请选择"
          size="large"
          clearable
          filterable
      ...
      <div class="search-section-item">
        ...
        >
          <el-option
            v-for="item in options.hospitalDeptList"
            :key="item.SectionId"
            :label="item.SectionName"
            :value="item.SectionId"
          />
      ...
      <div class="search-section-item">
        ...
        <el-input
          v-model.number="search.patientPhone"
          class="w-50 m-2"
          size="large"
          placeholder="请输入"
          maxlength="11"
      ...
      <div class="search-section-item">
        ...
        >
          <el-option
            v-for="item in options.procesStatusList"
            :key="item.id"
      ...
      <div class="search-section-item">
        ...
        >
          <el-option
            v-for="item in options.businesStatusList"
            :key="item.id"
            :label="item.statusName"
            :value="item.statusCode"
          />
      ...
      <div class="btn-group">
        ...
        <el-button size="large" class="btn-green-plain btn-height-34" @click="handle('',false,'门诊转出申请',1)">门诊转出申请</el-button>
      </div>
    </div>
    <div class="table-container">
      <el-table :data="state.tableData" border style="width: 100%" :show-overflow-tooltip="true">
        ...
        <el-table-column fixed="right" label="操作" width="240">
          <template #default="scope">
            <el-button v-if="getAction(scope.row.actionCodes,'SKIM')" link type="primary" size="small" @click="handle(scope.row.recordId,true,'门诊转出查看',2)">查看</el-button>
            <el-button v-if="getAction(scope.row.actionCodes,'EXIT')" link type="primary" size="small" @click="handle(scope.row.recordId,false,'门诊转出编辑',3)">编辑</el-button>
            <el-button v-if="getAction(scope.row.actionCodes,'REVIEW')" link type="primary" size="small" @click="handle(scope.row.recordId,false,'门诊转出审核')">审核</el-button>
...
// })
let search = store.outpatientServiceLeaveSearch
...
const clearSearch = () => {
  if (clearable(search)) {
    ...
    search.applyEndDate = '',  //申请结束时间
    search.patientName ='',  //患者姓名
    search.patientPhone = '',  //患者联系方式
    search.processStatus = '',  //流程状态
    search.businessStatus = '', //业务状态
    search.sourceDeptId = '', //转出科室
    search.targetDeptId = '', //转入科室
    search.sourceHospitalId = '', //转出医院
    search.targetHospitalId = '', //转入医院
    search.applyUserName =  "",
    search.pageNo = 1,
    search.pageSize = 10
...
const handle = (recordId = '', isSee = false, title, operateType = 1) => {
  ...
  window.parent.postMessage(
        JSON.stringify({
          method: "addTab",
          url: `${urlx}#/listDetail?type=1&transferType=MZ&recordId=${recordId}&isSee=${isSee}&operateType=${operateType}`,
          title
        }),
    "*"
  );
