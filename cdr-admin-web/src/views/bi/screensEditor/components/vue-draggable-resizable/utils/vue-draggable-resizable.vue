  ...
  methods: {
    ...
    checkParentSize() {
      if (this.parent) {
        ...
        // console.log(newParentWidth, newParentHeight);
    ...
    async handleUp(e) {
      ...
      if (this.resizing) {
        ...
        this.$emit('resizestop', this.left, this.top, this.width, this.height, this.targetItem);
      }
      if (this.dragging) {
        this.dragging = false;
        await this.conflictCheck();
        this.$emit('refLineParams', refLine);
        this.$emit('dragstop', this.left, this.top, this.targetItem);
      }
      this.resetBoundsAndMouseState();
      removeEvent(document.documentElement, eventsFor.move, this.handleResize);
    },
